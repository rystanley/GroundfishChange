...
load("C:/Users/User/Documents/School 2019-2020/Halibut/BIO/BNAM_Step1.RData")

##Removing monthly data from dataframe
avg_btmp = btmp %>% select(Longitude, Latitude, Depth, Winter_AVG, Summer_AVG, Annual_AVG, Year)

save(avg_btmp, file = "C:/Users/User/Documents/School 2019-2020/Halibut/BIO/BNAM_temp.RData")
...
hab$Habitat = ifelse(hab$Annual_AVG >= 3 & hab$Annual_AVG <= 8 & hab$Depth >= 50 & hab$Depth >= 25 & hab$Depth <= 200, 
         "Preffered", ifelse(hab$Annual_AVG >= 1.5 & hab$Annual_AVG <= 10 & hab$Depth >= 25 & hab$Depth <= 200,
                             "Good", "Not_suitable"))
...
NAFO = st_read("~/School 2019-2020/Halibut/BIO/Divisions/Divisions.shp")%>%
...
hab$Habitat = factor(hab$Habitat)
hab_group = hab %>% group_by(Year, ZONE, .drop = FALSE) %>% count(Habitat)

#Convert back to string because otherwise loop returns NAs and idk why
hab_group$Habitat = as.character(hab_group$Habitat)


#create character vector containg all unique habitat names
#create dataframe containing all zones by year
habitats = c(unique(habitat_group$Habitat))
...
for(i in 1:3){
  nam = habitats[i]
  total_hab[,i+2] =filter(habitat_group, Habitat == habitats[i])[,4]
}
...
save(prop_hab, file = "C:/Users/User/Documents/School 2019-2020/Halibut/BIO/BNAM_hab.RData")
