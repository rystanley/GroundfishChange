...
#Read in models and forcast data
...
unPred <- predict(I, newdata = uncertaintyData %>% 
                          select(longitude, latitude, region, depth, season,  btm_temp = btm_temp_PC) %>% 
                          mutate(season = factor(season),
                                 region = factor(region)), 
                        type=c("response"), se.fit = T)

upr <- plogis(unPred$fit + (1.96 * unPred$se.fit))
fit <-  plogis(unPred$fit)
lwr <-  plogis(unPred$fit - (1.96 * unPred$se.fit))
link2CI <- upr-lwr

responseData<- uncertaintyData %>% select(longitude, latitude) %>% mutate(uncertainty = link2CI)


u1 <- make_map(responseData, fill_name = "Uncertainty (CI Range) Response")

u1 + geom_sf(data = NAFO, alpha = 0, colour = "azure4")  +
  scale_x_continuous(limits = c(-71.62,-43), expand = c(0,0))+
  scale_y_continuous(limits = c(39,52), expand = c(0,0))+
  ...
  scale_fill_continuous(type = "viridis", guide = guide_colourbar(label.theme = element_text(angle = 90, size = 8, hjust = .5, vjust =  .5)))

ggsave(file = "Figures/Uncertainty_PC.jpeg", dpi = 600, width = 7.25, height = 5.4375, units = "in")
ggsave(file = "Figures/Uncertainty_PC.tiff", dpi = 600, width = 7.25, height = 5.4375, units = "in")

unlink <- predict(I, newdata = uncertaintyData %>% 
                         select(longitude, latitude, region, depth, season,  btm_temp = btm_temp_PC) %>% 
                           mutate(season = factor(season),
                                  region = factor(region)), 
                       type=c("link"), se.fit = T)


upr <- plogis(unlink$fit + (1.96 * unlink$se.fit))
fit <-  plogis(unlink$fit)
lwr <-  plogis(unlink$fit - (1.96 * unlink$se.fit))
linkCI <- upr-lwr

linkData <- uncertaintyData %>% select(longitude, latitude) %>% mutate(uncertainty = linkCI)

u2 <- make_map(linkData, fill_name = "Uncertainty (CI Range) Link")

u2 + geom_sf(data = NAFO, alpha = 0, colour = "azure4")  +
  scale_x_continuous(limits = c(-71.62,-43), expand = c(0,0))+
...
loopData <- forecast %>% 
  filter((region == "NF" & (season == 'Fall' | season == "Spring")) |
           ...
           (region == 'SS' & (season == 'Summer' | season == 'Winter')) |
           (region == 'SG' & season == 'Summer') |
           (region == 'USA' & (season == "Spring" | season == "Fall"))) %>% select(-ZONE, -month) %>% relocate(region, .before = depth)

mapdata <- forecast
#Mapping
#for(i in 1:length(loop_vars)){
  prob1 <- predict(I, newdata = mapdata %>% 
                    select(longitude, latitude, region, depth, season,  btm_temp = loop_vars[1]) %>% 
                    mutate(season = factor(season),
                           region = factor(region)), 
                  type=c("response"))
#  mapdata[i+3] <- prob
#}
  prob2 <- predict(I, newdata = mapdata %>% 
                    select(longitude, latitude, region, depth, season,  btm_temp = loop_vars[2]) %>% 
                    mutate(season = factor(season),
                           region = factor(region)), 
                  type=c("response"))
  prob3 <- predict(I, newdata = mapdata %>% 
                    select(longitude, latitude, region, depth, season,  btm_temp = loop_vars[3]) %>% 
                    mutate(season = factor(season),
                           region = factor(region)), 
                  type=c("response"))
  prob4 <- predict(I, newdata = mapdata %>% 
                    select(longitude, latitude, region, depth, season,  btm_temp = loop_vars[4]) %>% 
                    mutate(season = factor(season),
                           region = factor(region)), 
                  type=c("response"))
  prob5 <- predict(I, newdata = mapdata %>% 
                    select(longitude, latitude, region, depth, season,  btm_temp = loop_vars[5]) %>% 
                    mutate(season = factor(season),
                           region = factor(region)), 
                  type=c("response"))

%>% mutate(btm_temp_PC = prob1,
           btm_temp_F1_R45 = prob2,
           btm_temp_F1_R85 = prob3,
           btm_temp_F2_R45 = prob4,
           btm_temp_F2_R85 = prob5)

colnames(loopData) <- c("longitude", "latitude", "region", "season", loop_vars)

mapdata2 <-mapdata %>% select(longitude, latitude,region, season) %>% 
  mutate(btm_temp_PC = prob1,
                              btm_temp_F1_R45 = prob2,
                              btm_temp_F1_R85 = prob3,
                              btm_temp_F2_R45 = prob4,
                              btm_temp_F2_R85 = prob5,
                              delta_F1_R45 = btm_temp_F1_R45 - btm_temp_PC,
...
ggsave(file = "Figures/Fig5_IModel.tiff", dpi = 600, width = 7.25, height = 5.4375, units = "in")
ggsave(file = "Figures/Fig5_IModel.jpeg", dpi = 300, width = 7.25, height = 5.4375, units = "in")
...
ggsave(file = "Figures/Fig6_I_model.tiff", dpi = 600, width = 7.25, height = 5.4375, units = "in")
ggsave(file = "Figures/Fig6_I_model.jpeg", dpi = 300, width = 7.25, height = 5.4375, units = "in")
...
pChangeDivision <- mapdata2 %>%
 ...
 # select(-contains("delta")) %>% 
  ...
  ungroup() %>% filter(!is.na(ci))


pChangePlot <- pChangeDivision %>% mutate(lwr = percent_change - ci, 
                           upr = percent_change + ci,
                           sig = ifelse(upr >= 0 & lwr <= 0 | upr <= 0 & lwr >= 0, FALSE, TRUE))
...
ggplot(pChangePlot, aes(x = PCMean, percent_change))+
  geom_text(aes(label=ZONE, colour = sig),hjust=0.5, vjust=0.5)+
  geom_hline(yintercept = 0, linetype = 2)+
...
ggsave(file = "Figures/Fig7_I_model.tiff", dpi = 600, width = 7.25, height = 5.4375, units = "in")
ggsave(file = "Figures/Fig7_I_model.jpeg", dpi = 300, width = 7.25, height = 5.4375, units = "in")
...
write.excel(supp)
write.csv(supp, "output/SupplementTable.csv", row.names = F)
