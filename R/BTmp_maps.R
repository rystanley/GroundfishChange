...
m1 <- make_map(single_map,long.lim = c(-72.99797, -46),NAFO=NAFO)+ #get rid of non-halibut area on the plot
  ...
  theme(legend.position = "bottom", 
        text = element_text(size=36), axis.text.x = element_text(color = "grey20", size = 30, vjust = .5),
        axis.text.y = element_text(color = "grey20", size = 30, vjust = .5),
        axis.title.x = element_text(margin = margin(t = 10)), axis.title.y = element_text(margin = margin(b = 10)))

ggsave("output/map_temp2.tiff",m1,dpi=300,width=16,height=12,units="in")
